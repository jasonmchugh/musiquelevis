...
class WPMLTranslation {
	...
	public function woo_feed_switch_language( $language_code, $cookie_lang = true ) {
		...
		if ( is_array( $language_code ) && isset( $language_code['feedLanguage'] ) ) {
			...
			if ( isset( $language_code->feedInfo ) && is_array( $language_code->feedInfo ) && isset( $language_code->feedInfo['feedLanguage'] ) ) {
				$language_code = $language_code->feedInfo['feedLanguage'];
			}
		}
