...
class WC_API_Client_HTTP_Request {
	...
	public function dispatch() {
		...
		if ( null === $parsed_response ) {
			...
			throw new WC_API_Client_HTTP_Exception( sprintf( "Invalid JSON returned for %s.\n%s", $this->request->url,$this->response->body ), $this->response->code, $this->request, $this->response );
		}
		...
		if ( ! in_array( $this->response->code, array( '200', '201', '202' ) ) )
        {
            ...
            {
                if (array($parsed_response['errors']))
                {
                    $parsed_response['errors'] = $parsed_response['errors'][0];
                }
                
                $error_code    = $parsed_response['errors']['code'];
                $error_message = $parsed_response['errors']['message'];
			}
            else if (isset($parsed_response->code))
            {
        ...
        $this->response->jsonBody = $parsed_response;
	...
	protected function get_parsed_response( $raw_body )
    {
        ...
        */
        ...
        $raw_body = preg_replace("/^<br \/>.+\.php<\/b> on line <b>\d+<\/b><br \/>\n/s", "", $raw_body); //Bravad, for when we get warnings
