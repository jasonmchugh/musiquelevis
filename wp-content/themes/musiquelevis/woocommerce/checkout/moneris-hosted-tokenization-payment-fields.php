...
<fieldset>
	...
	if ( $tokens ) : ?>
		<p class="form-row form-row-wide">
			<a class="button" style="float:right;" href="<?php echo get_permalink( woocommerce_get_page_id( 'myaccount' ) ); ?>#wc-moneris-my-payment-methods"><?php echo wp_kses_post( apply_filters( 'wc_gateway_moneris_manage_my_payment_methods', __( "Manage My Cards", 'woocommerce-gateway-moneris' ) ) ); ?></a>
			<?php foreach( $tokens as $token ) : ?>
				...
				<label style="display:inline;" for="wc-moneris-payment-token-<?php echo esc_attr( $token->get_id() ); ?>"><?php printf( __( '%1$s se terminant par %2$s (expirant %3$s)', 'bravad' ), $token->get_image_url() ? '<img src="' . esc_url( $token->get_image_url() ) . '" />' : esc_html( $token->get_type_full() ), esc_html( $token->get_last_four() ), esc_html( $token->get_exp_month() . '/' . $token->get_exp_year() ) ); ?></label><br />
			<?php endforeach; ?>
