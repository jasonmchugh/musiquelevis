<?php
	 ...
	 * New order
	 */
	// add_action( 'woocommerce_new_order', 'send_succursale_invoice', 10 );
	// add_action( 'woocommerce_thankyou', 'send_succursale_invoice' );

	// function send_succursale_invoice( $order_id ) {
	// 	if ( current_user_can( 'manage_options' ) ) {
	...
	// 		    // $succ = $product->get_attribute( 'pa_succursale' );
	// 		    $succ = array_shift( wc_get_product_terms( $product->id, 'pa_succursale', array( 'fields' => 'slugs' ) ) );
	...
	// 		    send_invoice_email( $order_id, $email );
	// 		}
	// 	}
	// }

	// function send_invoice_email( $post_id, $email ) {
